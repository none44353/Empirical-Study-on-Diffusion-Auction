...
def getPrice(G, seller, bid, idom, H):
    ...
    for i in H:
        ...
        for jx in range(0, len(adj)):
            price[adj[jx]] = max(price[i], preMaxVal[jx], sufMaxVal[jx])
...
def IDM(G, bid, seller):
    idom = nx.immediate_dominators(G, seller)
    reachableNodes = list(filter(lambda i: nx.has_path(G, seller, i), G.nodes))
    price, maxAlpha = getPrice(G, seller, bid, idom, reachableNodes)
    mxBidder, diffusionSeq = getDiffSeq(seller, reachableNodes, idom, bid)
    ...
    return winner, price[diffusionSeq[1]]
...
def main():
    ...
    winner, Revenue = IDM(G, bid, seller)
